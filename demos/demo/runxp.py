...
in_dims = xp_dict["in_dims"]
rotations = xp_dict["rotations"]
uv_factors = xp_dict["uv_factors"]
n_blockss = xp_dict["n_blockss"]
...
for _ in range(repetition): # With next(seedg)
    conf_dicts += [
        dict(
            ...
            target = dict(func_name=func_name, in_dim=in_dim, func_sleep=func_sleep, rotation=rotation, n_blocks=n_blocks, uv_factor = uv_factor),
...
for index, conf in enumerate(conf_dicts):
    ...
    func_name = conf['target']["func_name"]
    in_dim = conf['target']["in_dim"]
    ...
    target_dir = os.path.join(
        "alldatas", algo_name, num_worker_name, func_name, "{}_{}".format(in_dim, budget_factor)
        )
