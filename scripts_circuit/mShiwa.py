...
def fun_mShiwa(obj_func, rand_init, in_dim, bounds_lower, bounds_upper, func_sleep, budget, seed, num_workers, inits, conn=None, timebound=None):
    ...
    while(not stopFlag and funCnt<budget):
        ...
        optimizer = ng.optimizers.Shiwa(parametrization=init_param, budget=budget, num_workers=num_workers)
        if not inits:
            pass
        else:
            restart_param = init_param.spawn_child()
            restart_param.value = inits[0]['candidate_value']
            optimizer = ng.optimizers.Shiwa(parametrization=restart_param, budget=budget, num_workers=num_workers)
