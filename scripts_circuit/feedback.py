...
def split_node(root_node, in_dim):
    nodes = [root_node]
...
def fun_feedback(algo_list, num_worker_list, pconn_list, in_dim, budget, pass_percent, pass_divide, seed, update_num_workers=True, using_portfolio=True):
    ...
    for algo in algo_list:
        if is_multi_start(algo):
            active_budget_dict[algo] = num_worker_dict[algo]*2*in_dim
        else:
            active_budget_dict[algo] = 2*in_dim
    ...
    def train_multitask_model(for_multi_mean_list, device):
        ...
        try:
            model.train()
            model.train_hypers(train_iter=2000) 
    ...
    def train_a_model(y_list, algo, budget, cur_budget, device):
        ...
        # print("in train_a_model, len train_y", len(train_y))
        # print("in train_a_model, len train_x", len(train_x))
        model = ExactGPModel(
            train_x,
            train_y, 
            ).to(device)
        try:
            model.train()
            model.train_hypers(train_iter=1000) 
        except RuntimeError as e:
            logging.info(repr(e))
    ...
    while(cur_budget < budget):
        ...
        for algo, pconn in zip(algo_list, pconn_list):
            ...
            records = pconn.recv()
            logging.info("algo {} records received".format(algo))
        ...
        for algo, pconn, send_flag in zip(algo_list, pconn_list, send_list):
            if send_flag:
                ...
                logging.info("algo {} send_flag {}".format(algo, send_flag))
                try:
                    if (algo in terminate_algo_list) and (algo not in done_terminate_algos):
                    ...
                    elif algo not in done_terminate_algos:
        ...
        restart_flag = False
        if round_cnt or cur_budget > budget-in_dim:
            ...
            for algo in algo_list:
                if algo in model_dict.keys(): # no_terminated_algo_list
                    ...
                    temp_budget = min(cur_budget+0.5*(cur_budget-begin_budget), budget)
            ...
            try:
                ...
                if np.any(np.array([i for i in tol_cnt_dict.values()]) >= 3):
                    ...
                    try:
                        ...
                        leaf_nodes = split_node(root_node, in_dim)
                    ...
                    for algo in temp_poor_algos:
                        ...
                        if len(new_terminate_algo_list) < len(restart_algo_list) - 1:
                            ...
                            # 1: no restart 2: one chance to restart 4: three chances to restart ...
                            if is_reinitialize_start(algo):
                                if poor_cnt_dict[algo] >= 4:
                    ...
                    # Some or even all these algos may be terminated
                        ...
                        if using_portfolio: # Using data (end_mean, end_stddev, end_covar)
                            try:
                                ...
                                reward_array = (best_one - mean_array) / unit_array
                        ...
                        if not using_portfolio: # Using data (PI_list)
                            ## Normalized unit PI
