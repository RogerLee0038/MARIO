...
if not os.path.exists(npy_file):
    ...
    for f_name in func_list:
        ...
        for opt_name in optimizer_list:
            ...
            sub_df = grouped.get_group((opt_name, f_name))
            print(sub_df['dataset_loc'].unique()[0])
            # print(sub_df)
            print("length of the sub-dataframe:", len(sub_df))
...
for sort_name, fill_color in zip(
    ...
    if hline_flag:
        ...
        #ax.axhline(hline_val, 10/fb_factor, 1, color=p[0].get_color(), linestyle='--', linewidth=5)
        ax.axhline(hline_val, 5/fb_factor, 1, color='C3', linestyle='--', linewidth=7.5)
        ax.text(5, hline_val+0.75, "{:.2f}X".format(fb_factor/cross_x), fontsize=44, fontweight='bold', color=mine_color)

ax.set_xlim(0, fb_factor)
ax.set_xlabel('Normalized number of evaluations', fontdict={'size':32, 'weight':'bold'})
...
plt.yticks(size=15)
plt.grid()
plt.savefig("pp_tan.pdf", bbox_inches='tight')
