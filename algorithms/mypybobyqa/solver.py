...
def solve_main(objfun, x0, args, xl, xu, npt, rhobeg, rhoend, maxfun, nruns_so_far, nf_so_far, nx_so_far, nsamples, params,
               diagnostic_info, scaling_changes, f0_avg_old=None, f0_nsamples_old=None, do_logging=True, print_progress=False, initials=None):
    # Evaluate at x0 (keep nf, nx correct and check for f small)
    # ch = logging.StreamHandler()
    # ch.setLevel(logging.INFO)
    # module_logger.addHandler(ch)
    format_here = "%(asctime)s - %(levelname)s - %(lineno)s\nmpybobyqa: %(message)s\n"
    logging.basicConfig(level=logging.INFO, stream=sys.stdout, format=format_here)
    ...
    if do_logging:
        module_logger.info("init rhobeg " + str(rhobeg))
    ...
    if not initials:
        ...
        num_directions = npt - len(initials)
    if do_logging:
        module_logger.info("init num_directions " + str(num_directions))
    if num_directions >= 1:
    ...
    if initials:
        shift_num = 1 + num_directions
        control.initialise_available(shift_num, initials)
...
def solve(objfun, x0, args=(), initials=None, bounds=None, npt=None, rhobeg=None, rhoend=1e-8, maxfun=None, nsamples=None, user_params=None,
    ...
    if exit_info is not None:
        ...
        if do_logging:
            module_logger.error("in pybobyqa solve" + exit_msg)
