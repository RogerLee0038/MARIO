...
def eval_objective(objfun, x, args=(), verbose=True, eval_num=0, pt_num=0, full_x_thresh=6, check_for_overflow=True):
    ...
    if verbose:
        if len(x) < full_x_thresh:
            module_logger.debug("Function eval %i at point %i has f = %.15g at x = " % (eval_num, pt_num, f) + str(x))
        else:
            module_logger.debug("Function eval %i at point %i has f = %.15g at x = [...]" % (eval_num, pt_num, f))
