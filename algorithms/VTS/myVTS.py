...
class myVTS(object):
    ...
    def __init__(self, lb, ub, dims, ninits, init, with_init, initials, func, iteration, Cp = 0, leaf_size = 20, kernel_type = "rbf", use_cuda = False, set_greedy=True):
    ...
    def evaluate_fun(self, sample):
        ...
        if value < self.curt_best_value:
            ...
            self.curt_best_sample = np.array(sample) 
        self.sample_counter += 1
        dic = {'X':sample, 'Y':np.array([value])}
    ...
    def init_train(self):
        ...
        if self.initials is not None:
            for sample, value in self.initials[1:]:
                dic = {'X':sample, 'Y':np.array([value])}
                self.samples.append( dic )
            initial_cnt = len(self.initials) - 1
        if self.with_init:
            init_points = latin_hypercube(self.ninits-1-initial_cnt, self.dims)
