    ...
    def optimize(self,x0, eps = 1e-2, damping = False):
        ...
        if len(idx.shape)>0 and idx.shape != torch.Size([0]):
            ...
            for k in range(self.max_iter):
                ...
                self.x.add_(s)
                if self.f(self.x).item() < new_f:
                    new_f = self.f(self.x).item()
...
def acq_min_msp(f, gradf, x_init, bounds, n_warmup=10000, n_iter=1):
    ...
    min_acq = ys[idx[0]]
    x_min = x_tries[idx[0]]
    print("in acq_min_msp x_init: {}".format(x_init))
    print("in acq_min_msp x_min: {}".format(x_min))
    x_bak = x_min.clone().detach()
    ...
    for k in range(n_iter):
        ...
        if not (True in torch.isnan(xopt)):
            print("xopt", xopt)
            print("yopt", yopt)
            print("xmin", x_min)
            print("min_acq", min_acq)
            if min_acq is None or yopt <= min_acq:
            # if (min_acq is not None) and (yopt is not None) and (yopt <= min_acq):
                x_min = xopt
                min_acq = yopt
    print("out acq_min_msp x_min: {}".format(x_min))
    if (True in torch.isnan(xopt)):
        x_min = x_bak
        min_acq = f(x_min)
