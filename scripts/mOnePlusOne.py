...
def fun_mOnePlusOne(obj_func, rand_init, in_dim, bounds_lower, bounds_upper, func_sleep, budget, seed, num_workers, inits, conn=None, timebound=None):
    ...
    while(not stopFlag and funCnt<budget):
        ...
        optimizer = ng.optimizers.OnePlusOne(parametrization=init_param, budget=budget)
        if not inits:
            pass
        else:
