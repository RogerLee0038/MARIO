...
for _ in range(repetition): # With next(seedg)
    conf_dicts += [
        dict(
            ...
            target = dict(func_name="My" + func_name, func_sleep=func_sleep, in_dim=in_dim),
...
for index, conf in enumerate(conf_dicts):
    ...
    proc.wait()
    with open("results/summary.pkl", "rb") as f2:
...
#df.to_csv("{}_po.csv".format(exp_name), index=False)
