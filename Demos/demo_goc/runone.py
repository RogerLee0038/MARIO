...
with open("run.log", "w") as out_file:
    proc = psutil.Popen(["python3", os.path.join(PO_SRC, "scripts", "run_goc_asy.py")], stdout = out_file)
    while(True):
        if os.path.exists("finish"):
            print("run_goc_asy.py done in runone.py")
            proc.terminate()
            break
        else:
            status = proc.status()
            print("in runone.py, run_goc_asy.py is running, status {}".format(status))
            time.sleep(10)
os.chdir(cur_dir)
