...
class Controller(object):
    ...
    def initialise_available(self, shift_num, initials):
        for k, tup in enumerate(initials[1:]):
            x = tup[0]
            f_list = [tup[1]]
            # expect step, not absolute x
            self.model.change_point(shift_num + k, x - self.model.xbase, f_list[0], allow_kopt_update=False)  
